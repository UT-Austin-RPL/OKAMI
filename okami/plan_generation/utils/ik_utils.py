...
from okami.plan_generation import ROOT_PATH

class IKWrapper:
    def __init__(self):
        self.model = mujoco.MjModel.from_xml_path(os.path.join(ROOT_PATH, "../third_party/mjctrl/franka_emika_panda/scene.xml"))
    ...
    def ik_trajectory_from_T_seq(self, T_seq, start_joint_positions, verbose=True):
        ...
        for i in range(len(T_seq)):
            T = T_seq[i]
            # centroid_T = np.eye(4)
            # centroid_T[:3, 3] = centroid_seq[i]
            new_T = T @ new_T
            # new_T = T @ (np.linalg.inv(centroid_T) @ new_T)
            # new_T = centroid_T @ new_T
        ...
        return predicted_joints_seq, new_T_seq, quat_seq
    
    def ik_trajectory_from_R_t_seq(self, R_seq, t_seq, start_joint_positions, verbose=True):
        assert(len(start_joint_positions) == 7), "start_joint_positions should be a list of 7 elements"
        predicted_joints_seq = [np.array(start_joint_positions)]
        ...
        for R, t in zip(R_seq, t_seq):
            new_pos = t + new_pos
            new_pos_seq.append(new_pos)


        new_T_seq = []
        for new_R, new_t in zip(new_R_seq, new_pos_seq):
            new_T = np.eye(4)
            new_T[:3, :3] = new_R
            new_T[:3, 3] = new_t
            new_T_seq.append(new_T)
    ...
    def visualize_joint_sequence(self, joint_sequence, loop=False, fps=30):
        ...
        with mujoco.viewer.launch_passive(
                ...
                while viewer.is_running():
                    for joint_conf in joint_sequence:
                        ...
                        mujoco.mj_step(self.model, self.data, 1)
                        # print(data.site(gripper_site_id).xpos)
                        # print(data.site(gripper_site_id).xmat.reshape(3, 3))
                        viewer.sync()
                        time.sleep(1/fps)
                        gripper_site_id = self.model.site("grip_site").id
