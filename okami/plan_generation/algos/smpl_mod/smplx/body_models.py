...
class SMPL(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        vertices, joints, joint_transforms = lbs(
            betas,
            full_pose,
            self.v_template,
            self.shapedirs,
        ...
        if apply_trans:
            ...
            joint_transforms[:, :, :3, 3] += transl.unsqueeze(dim=1)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if data_struct is None:
            # Load the model
            if osp.isdir(model_path):
                model_fn = f"SMPLH_{gender.upper()}.{ext}"
                smplh_path = os.path.join(model_path, model_fn)
            else:
                smplh_path = model_path
            assert osp.exists(smplh_path), f"Path {smplh_path} does not exist!"

            print("smplh_path=", smplh_path)
...
class SMPLX(SMPLH):
    """SMPL-X (SMPL eXpressive) is a unified body model, with shape parameters trained jointly for
    the face, hands and body.

    SMPL-X uses standard vertex based linear blend skinning with learned corrective blend shapes,
    has N=10475 vertices and K=54 joints, which includes joints for the neck, jaw, eyeballs and
    fingers.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert osp.exists(smplx_path), f"Path {smplx_path} does not exist!"
    ...
    def forward(
        ...
        **kwargs,
        ...
        output = MANOOutput(
            ...
            joint_transforms=joint_transforms if return_verts else None,
            betas=betas,
            global_orient=global_orient,
            hand_pose=hand_pose,
            full_pose=full_pose if return_full_pose else None,
...
class FLAME(SMPL):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert osp.exists(flame_path), f"Path {flame_path} does not exist!"
        if ext == "npz":
...
def build_layer(
    ...
    model_path: str
        ...
        where each subfolder contains the different types, i.e.:
        model_path:
        |
        |-- smpl
            |-- SMPL_FEMALE
