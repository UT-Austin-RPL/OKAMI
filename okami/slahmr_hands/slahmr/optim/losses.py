...
class MotionLoss(SMPLLoss):
    ...
    def forward(
        ...
        # prior to keep latent pose likely
        if "hand_pose" in pred_data and self.loss_weights["pose_prior"] > 0.0:
            B, T, _ = pred_data["hand_pose"].shape
            async_mask = (
                torch.arange(T, device=device)[None].expand(B, -1)
                < valid_mask.sum(dim=1)[:, None]
            )
            cur_loss = pose_prior_loss(pred_data["hand_pose"], async_mask)
            loss += self.loss_weights["pose_prior"] * cur_loss
            stats_dict["hand_pose_prior"] = cur_loss

        if "hand_pose" in pred_data and self.loss_weights["hand_pose_smooth"] > 0.0:
            hand_pose = pred_data["hand_pose"]  # (T, 3, 3)
            cur_loss = hand_pose_smoothness_loss(hand_pose[:,1:], hand_pose[:,:-1])
            loss += self.loss_weights["hand_pose_smooth"] * cur_loss
            stats_dict["hand_pose_smooth"] = cur_loss
