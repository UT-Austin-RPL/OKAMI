...
class MovingSceneModel(BaseSceneModel):
    ...
    def init_first_state(self, obs_data, param_dict, data_fps):
        ...
        if self.async_tracks and "track_interval" in obs_data:
            ...
            param_dict = select_dict_segments(
                param_dict, start, end, names=["trans", "root_orient", "latent_pose", "hand_pose"]
            )
        ...
        hand_pose = param_dict["hand_pose"]  # (B, T, 2*C)
        ...
        self.params.set_param("hand_pose", hand_pose)
        ...
        init_latent = self.infer_latent_motion(
            trans, root_orient, body_pose, betas, hand_pose, data_fps
        ).detach()  # (B, T-1, D)
    ...
    def estimate_prior_velocities(self, trans, root_orient, body_pose, betas, hand_pose, data_fps):
        """
        ...
        """
        with torch.no_grad():
            self.update_world2prior(trans, root_orient, body_pose, betas, hand_pose)
            trans, root_orient = self.apply_world2prior(
                trans, root_orient, body_pose, betas, hand_pose
            )
            smpl_results = self.pred_smpl(trans, root_orient, body_pose, betas, hand_pose)
        return estimate_velocities(
            trans, root_orient, smpl_results["joints3d"], data_fps
        )

    def update_world2prior(self, trans, root_orient, body_pose, betas, hand_pose):
        B = trans.shape[0]
        smpl_data = self.pred_smpl(
            trans[:, :1], root_orient[:, :1], body_pose[:, :1], betas, hand_pose[:, :1],
        )
    ...
    def apply_world2prior(self, trans, root_orient, body_pose, betas, hand_pose, inverse=False):
        """
        ...
        cur_smpl_data = self.pred_smpl(trans, root_orient, body_pose, betas, hand_pose)
    ...
    def infer_latent_motion(
        self, trans, root_orient, body_pose, betas, hand_pose, data_fps, full_forward_pass=False
    ):
        """
    ...
    def rollout_smpl_steps(self, num_steps=-1):
        ...
        preds = self.pred_smpl(
            res["trans"], res["root_orient"], res["pose_body"], res["betas"], res["pose_hand"],
        )
    ...
    def rollout_latent_motion(
        ...
        trans, root_orient, betas, hand_pose = (
            self.params.trans,
