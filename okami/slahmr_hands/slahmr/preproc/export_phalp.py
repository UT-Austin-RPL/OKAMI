...
def export_vitpose_keypoints(res_path, target_dir):
    ...
    for frame, track_data in tracklet_data.items():
        ...
        extra_data = track_data["extra_data"]
        ...
        for i, tid in enumerate(tids):
            ...
            kp_dict = {
                "people": [
                    {
                        "pose_keypoints_2d": extra_data[i]["vitpose"].tolist(),
...
def unpack_frame(track_data):
    ...
    for i, tid in enumerate(track_data["tid"]):
        ...
        extra_data = track_data["extra_data"][i]
        ...
        left_hand_pose = extra_data["left_hand_pose"]
        left_hand_pose_aa = np.stack(
            [cv2.Rodrigues(x)[0].squeeze() for x in left_hand_pose], axis=0
        )  # (15, 3)
        # l-r flipping in aa
        left_hand_pose_aa[:,1:] *= -1
        right_hand_pose = extra_data["right_hand_pose"]
        right_hand_pose_aa = np.stack(
            [cv2.Rodrigues(x)[0].squeeze() for x in right_hand_pose], axis=0
        )  # (15, 3)
        ...
        out_dict[tid] = {
            ...
            "right_hand_pose": right_hand_pose_aa.tolist(),
            "left_hand_pose": left_hand_pose_aa.tolist(),
