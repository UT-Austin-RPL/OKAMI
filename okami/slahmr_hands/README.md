...
3. Download models from [here](https://drive.google.com/file/d/1GXAd-45GzGYNENKgQxFQ4PHrBp8wDRlW/view?usp=sharing). Run
    ...
    All models and checkpoints should have been unpacked in `_DATA`. Also download the SMPLX model and put SMPLX_NEUTRAL.npz in `_DATA/body_models/smplx`. Moreover, donload the MANO model and put MANO_RIGHT.pkl in '_DATA/data/mano/'.

    Moreover, you need to download the HaMeR data:
    ...
    Finally, for a short demo, you can download:
    ```
...
```
python run_opt.py data=video data.seq=022691_mpii_test data.root=/full_path/slahmr/demo run_opt=True run_vis=True
```

This is from the demo video you downloaded above. You can change that with another video instead.

If you want to only run the fast version (less accurate, more robust), comment out Lines 23-27 in file `slahmr/confs/optim.yaml`.
