...
from robosuite.models.objects import BoxObject, ShortCabinetObject, WoodenCabinetObject, BottleObject, BowlObject, SimpleBottle
...
class HumanoidPour(ManipulationEnv):
    ...
    def __init__(
        ...
        self.table_offset = np.array((0, 0, 1.10 - 0.15)) # TODO: delete -0.15
    ...
    def _load_model(self):
        ...
        self.model = ManipulationTask(
            ...
            mujoco_objects=[self.bottle, self.bowl],
        )
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            def bottle_pos(obs_cache):
                return self.sim.data.body_xpos[self.bottle_body_id]
            
            @sensor(modality=modality)  
            def bottle_quat(obs_cache):
                return convert_quat(self.sim.data.body_xquat[self.bottle_body_id], to="xyzw")
            
            @sensor(modality=modality)
            def bowl_pos(obs_cache):
                return self.sim.data.body_xpos[self.bowl_body_id]
            
            @sensor(modality=modality)
            def bowl_quat(obs_cache):
                return convert_quat(self.sim.data.body_xquat[self.bowl_body_id], to="xyzw")
            ...
            sensors = [gripper_to_bottle_pos, base_pos, bottle_pos, bottle_quat, bowl_pos, bowl_quat]
    ...
    def _check_success(self):
        """
        Check if the bottle is in the upper position of bowl, and the angle is ready to pour things into the bowl.

        Returns:
            bool: True if drawer has been lifted
        """
        
        bowl_center = self.sim.data.body_xpos[self.bowl_body_id]
        bowl_center_xy = bowl_center[:2]

        bottle_center = self.sim.data.body_xpos[self.bottle_body_id]

        bottle_lid_direction = self.sim.data.body_xquat[self.bottle_body_id]
        # change (w, x, y, z) to (x, y, z, w)
        bottle_lid_direction = np.array([bottle_lid_direction[1], bottle_lid_direction[2], bottle_lid_direction[3], bottle_lid_direction[0]])

        v = np.array([0, 0, 0.0325])
        rot_mat = R.from_quat(bottle_lid_direction)
        v_new = rot_mat.apply(v)

        bottle_lid_pos = bottle_center + v_new

        bottle_lid_pos_xy = bottle_lid_pos[:2]
        xy_err = np.linalg.norm(bottle_lid_pos_xy - bowl_center_xy)

        return (xy_err < 0.06) and (v_new[2] < 0)
