...
from robosuite.controllers.gripper.gripper_controller import GripperController
...
class GripperJointPositionController(GripperController):
    ...
    def __init__(
        ...
        **kwargs,  # does nothing; used so no error raised when dict is passed with extra terms used previously
        ...
        print("joint_indexes=", joint_indexes)
        print("actuator_range=", actuator_range.shape)
        print("actuator_range shape:", len(actuator_range[0]))

        # Control dimension
        # self.control_dim = len(joint_indexes["joints"])
        self.control_dim = len(joint_indexes["actuators"])
        self.actuator_indexes = joint_indexes["actuators"]
        print("actuator_indexes: ", self.actuator_indexes, joint_indexes["joints"])
    ...
    def set_goal(self, action, set_qpos=None):
        ...
        self.goal_qpos = set_goal_position(
            scaled_delta, self.joint_pos[self.actuator_indexes], position_limit=self.position_limits, set_pos=set_qpos
        )
    ...
    def run_controller(self):
        ...
        position_error = desired_qpos - self.joint_pos[self.actuator_indexes]
        vel_pos_error = -self.joint_vel[self.actuator_indexes]
