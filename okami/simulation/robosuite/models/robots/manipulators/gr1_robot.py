...
class GR1FloatingBody(GR1):
    ...
    def init_qpos(self):
        ...
        init_qpos[3:6] = head_init
        torso_init = np.array([0.0, 0.22, 0.0])
        init_qpos[0:3] = torso_init
        right_arm_init = np.array([0., 1.57, 0., -1.57, 0., 0., 0.]) # np.array([0.0, 0.2, 0.189, -1.43, 0.864, 0.0, 0.0])
        init_qpos[6:13] = right_arm_init
        left_arm_init = np.array([0., -1.57, 0., 1.57, 0., 0., 0.]) # np.array([0.0, -0.2, -0.189, 1.06, -0.581, 0.0, 0.0])
    ...
    def base_xpos_offset(self):
        return {
            "bins": (-0.5, -0.1, 0.97),
            "empty": (-0.29, 0, 0.97),
            "table": lambda table_length: (-0.26 - table_length / 2, 0, 0.82), # 0.97
        }
